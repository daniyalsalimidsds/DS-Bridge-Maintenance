...
public class ScoringReleaseInstrumentationTest {
    ...
    @Test public void scoredFinalizationRejectsPendingRowsAndRequiresEvidence() throws Exception {
        ...
        try(AppDb db=new AppDb(c)){
            ...
            JSONObject audit=new JSONObject().put("id","qa-score-audit").put("action","legacy-scoring-finalization-test").put("kind","inspections").put("eid","qa-score-final");
    ...
    private static void ready(ActivityScenario<MainActivity> s)throws Exception{long until=SystemClock.elapsedRealtime()+15000;while(SystemClock.elapsedRealtime()<until){if("ready".equals(eval(s,"document.readyState==='complete'&&typeof BridgeScoring==='object'&&typeof newInspection==='function'&&window.Governance?.initialized?'ready':'wait'")))return;SystemClock.sleep(150);}fail("App did not become ready");}
    private static void authenticateAdmin(ActivityScenario<MainActivity> s)throws Exception{assertEquals("started",eval(s,"(()=>{window.__qaAuth='wait';document.getElementById('authPin').value='135790';document.getElementById('authPinConfirm').value='135790';submitAuthentication().then(()=>window.__qaAuth=Governance.currentUser()?.id||'failed').catch(()=>window.__qaAuth='failed');return 'started'})()"));long until=SystemClock.elapsedRealtime()+10000;while(SystemClock.elapsedRealtime()<until){if("user-admin".equals(eval(s,"window.__qaAuth||'wait'")))return;SystemClock.sleep(100);}fail("Administrator authentication failed");}
