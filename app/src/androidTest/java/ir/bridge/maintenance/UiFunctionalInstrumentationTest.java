...
public class UiFunctionalInstrumentationTest {
 @Test public void addUserActuallyPersistsAndCanBecomeActive() throws Exception {
  try(ActivityScenario<MainActivity> s=ActivityScenario.launch(MainActivity.class)){
   waitReady(s);
   String uid=eval(s,"(()=>{const u={id:'qa-user-'+Date.now(),name:'کاربر آزمون QA',role:'بازرس',active:true};dbSave('users',u);return u.id})()");
   assertEquals("saved",eval(s,"dbList('users').some(x=>x.id==='"+uid+"'&&x.name==='کاربر آزمون QA')?'saved':'missing'"));
   eval(s,"setActiveUser('"+uid+"');'ok'");
   assertEquals(uid,eval(s,"getActiveUser()?.id||''"));
  }
 }
 @Test public void bridgeSelectionIsFirstAndRemovedRailwayFieldsDoNotExist() throws Exception {
  try(ActivityScenario<MainActivity> s=ActivityScenario.launch(MainActivity.class)){
   waitReady(s);eval(s,"(()=>{const fs=BRIDGE_PROFILE_SCHEMA.sections.flatMap(x=>x.fields),fid=l=>fs.find(x=>x.label===l).id,b={id:'qa-ui-bridge',name:'پل رابط',code:'UI-01',use:'راه‌آهن',profile:{}};b.profile[fid('نام پل')]=b.name;b.profile[fid('کد پل')]=b.code;b.profile[fid('کاربری اصلی')]=b.use;b.profile[fid('مصالح اَبَرسازه')]='فولادی';b.profile[fid('نوع روسازی')]='راه‌آهن - ریل روی تراورس و بالاست متکی بر عرشه';dbSave('bridges',b);newInspection(b.id);return 'ok'})()");
 ...
 private static void waitReady(ActivityScenario<MainActivity> s)throws Exception{long deadline=SystemClock.elapsedRealtime()+15000;String v="";while(SystemClock.elapsedRealtime()<deadline){v=eval(s,"document.readyState==='complete'&&typeof dbList==='function'&&typeof newInspection==='function'&&typeof setActiveUser==='function'?'ready':'wait'");if("ready".equals(v))return;SystemClock.sleep(150);}assertEquals("ready",v);}
