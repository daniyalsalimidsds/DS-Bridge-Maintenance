...
public class MigrationBackupInstrumentationTest {
 @Test public void v1DatabaseUpgradesWithoutLosingLegacyEntity() throws Exception {
  ...
  try(AppDb db=new AppDb(c)){SQLiteDatabase upgraded=db.getWritableDatabase();assertEquals(3,upgraded.getVersion());try(Cursor cur=upgraded.rawQuery("SELECT json FROM entities WHERE kind='inspections' AND eid='legacy-1'",null)){assertTrue(cur.moveToFirst());assertTrue(cur.getString(0).contains("legacy-1"));}try(Cursor cur=upgraded.rawQuery("SELECT name FROM sqlite_master WHERE type='table' AND name='media'",null)){assertTrue(cur.moveToFirst());}try(Cursor cur=upgraded.rawQuery("SELECT name FROM sqlite_master WHERE type='table' AND name='audit_events'",null)){assertTrue(cur.moveToFirst());}}
 }
